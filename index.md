...
In order to avoid friction by violating [Conway's Law](https://en.wikipedia.org/wiki/Conway%27s_law) for a big enough problem in a big enough organisation, you need to have at least one system per team. If you develop [consumer facing websites](#consumer-facing-websites) and value both ["Time To Interaction" performance](#time-to-interaction-is-important) and [long-term evolvability from heterogeneity](#long-term-evolvability-comes-from-heterogeneity), you should use server-side rendering. [Integrating on data](#data) is safe, but don't allow for economies of scale (the effort is duplicated by each team). [Integrating on code](#code) creates the need for release trains, which is costly and hurts agility. [Integrating on content](#content) (HTML) allow the team to not duplicate their efforts, while decoupling their release cycles.

Integrating on content is also called [transclusion](https://en.wikipedia.org/wiki/Transclusion) and we like the declarative approaches of translusion the best. One server-side transclusion technique is [Edge-Side Includes](#edge-side-includes) (ESI), which is supported by a few CDNs and some caching HTTP reverse proxies. ESI is infrastructure heavy, so it might hurt your initial project delivery speed. It can introduce performance risks, if one of the servers are having performance problems. ESI is something that we can easily refactor to later, if wanted. Declarative [Client-Side Includes](#client-side-includes) (CSI) allows for better initial project delivery speed, and together with HTTP/2 the performance is not too bad for many scenarios. However, CSI introduces some challenges with loading service local JavaScript, but that challenge is solvable with [script loaders](#local-scripts).
